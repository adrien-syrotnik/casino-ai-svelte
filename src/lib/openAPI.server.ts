...
export async function GenerateAllPromptsForGenerateImages(theme: string, apiKey: string) {
    ...
    const completion = await openai.chat.completions.create({
        messages: [
            ...
            { role: "user", content: "Generate 9 prompts for generating images using DALL-E 3. Here are the symbols: " + AllImageToGenerate.map(s => s.symbol).join(", ") + ". For the symbols 'm1', 'm2', 'm3', 'm4', provide unique prompt that vary in complexity, indicating the size and importance of the symbol, from medium to huge, without including textual elements in the imagery. . For the symbols '10', 'J', 'Q', 'K', 'A', provide a prompt that allow to generate image that represent the letter and number, they must be similar design. Finally, for the symbol 'wild', provide a prompt that allow to generate a dynamic and eye-catching sprite, incorporating vibrant colors and intricate details to evoke a sense of excitement, the word 'wild' should be prominently displayed within or around the symbol, using a bold and stylized font that complements the overall design. Consider the perspective by opting for a close-up view of the wild symbol, ensuring that its details are clearly visible. Illuminate the sprite with a dramatic and focused light source, enhancing the overall visual impact. The composition should be well-balanced, with the wild symbol positioned centrally against the black background. The JSON format must be: {'10': 'prompt', 'J': 'prompt', 'Q': 'prompt', 'K': 'prompt', 'A': 'prompt', 'm1': 'prompt', 'm2': 'prompt', 'm3': 'prompt', 'm4': 'prompt', 'wild': 'prompt'}. Here is the theme for the symbols: " + theme + "." },
        ],
        // messages: [
        //     {
        //         role: "system",
        //         content: "You are an AI specialized in generating JSON-formatted prompts for creating images for an online slot machine. For each symbol icon, provide detailed, unique, and themed instructions that do not include text within the image. Work with the specified theme to create the following symbols: 'm1', 'm2', 'm3', 'm4', '10', 'J', 'Q', 'K', 'A', and 'wild'. Ensure that the prompts for 'm1', 'm2', 'm3', and 'm4' result in images with proportionally increasing sizes and complexity. The '10', 'J', 'Q', 'K', 'A' symbols should be rendered in a cohesive design style. The 'wild' symbol prompt should describe a vibrant, detailed design that includes the word 'wild' in a stylized font and has a dynamic, visually striking appearance. Output your instructions as a JSON object with the following format: {'symbol_key': 'prompt instruction'}."
        //     },
        //     {
        //         role: "user",
        //         content: `Create JSON-formatted prompts for DALL-E 3 to generate 9 themed symbols for a slot machine. The symbols include '10', 'J', 'Q', 'K', 'A' ,'m1', 'm2', 'm3', 'm4', and 'wild'. For 'm1' through 'm4', each prompt must vary in complexity, indicating the size and importance of the symbol, from medium to huge, without including textual elements in the imagery. Symbols '10', 'J', 'Q', 'K', 'A' should be visually unified in design style and easily associated with their respective letters or numbers. The 'wild' prompt must encapsulate a dynamic, alluring design, prominently featuring the word 'wild' in an artistic and compatible font. The render must be close-up, vividly detailed, and placed against a dark background to enhance focus and visual appeal. Each prompt should align with the theme: '${theme}'. Format the output as follows: {'10': 'prompt', 'J': 'prompt', 'Q': 'prompt', 'K': 'prompt', 'A': 'prompt', 'm1': 'prompt', 'm2': 'prompt', 'm3': 'prompt', 'm4': 'prompt', 'wild': 'prompt'}.`
        //     }
        // ],
